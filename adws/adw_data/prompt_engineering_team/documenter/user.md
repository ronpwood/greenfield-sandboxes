...
Your part in this run: tell the next engineer what the team built for the request in `prompt`, and why.

1. Read the full diff at `previous_envelope.diff_path`, plus any changed file that needs context.
2. Read `<context_handoff_dir>/plan.md`: `## What we're solving for`, `## Team notes`, `## Amendments`.
3. Write the write-up to `<context_handoff_dir>/document.md`. Cover: what changed and why it matters, the files that carry it, any amendment that moved the spec and why, and how to use or verify it.
4. Copy that file into the repo under `app_docs/`:
...
5. Emit your `Report` JSON, declaring BOTH paths in `artifacts`.
