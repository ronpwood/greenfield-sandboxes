...
Your part in this run: write the team's spec for the work described in `prompt`.

1. Write the spec to `<context_handoff_dir>/plan.md`, in the eight-section form from your
   instructions. This is the copy the whole team reads and annotates. Start with
   `## What we're solving for`, and give `## Expected values` the most care.
2. Copy that file into the repo under `specs/`:
   ...
   - That repo copy is the one the team's frozen sections are measured against. Copy it once, after `plan.md` is final.
...
```json
{
  ...
  "notes_for_next_agent": "<what the team must know that the spec does not already say>"
}
```
